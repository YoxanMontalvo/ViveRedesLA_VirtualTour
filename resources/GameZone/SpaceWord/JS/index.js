import { getRandomValue, getRandomWordArray, removeAllChildNodes, createStars } from '../../SpaceWord/JS/utils.js';
import { getWordsArrayBySettingsOrPlatform } from '../../SpaceWord/JS/data.js';
...
const shoot = () => {
    const wordElement = document.querySelector(`#${currentlyWrittingWord}`);
    if (wordElement) {
        const { x, y } = wordElement.getBoundingClientRect();
...
const moveShipHorizontally = () => {
    const wordElement = document.querySelector(`#${currentlyWrittingWord}`);
    if (wordElement) {
        const { x } = wordElement.getBoundingClientRect();
...
const detectKeyTyping = () => {
    document.addEventListener('keydown', function (event) {
        ...
        if (foundWord && !currentlyWrittingWord) {
            const wordElement = document.querySelector(`#${foundWord}`);
            const { top } = wordElement?.getBoundingClientRect() || {};
            if (top && top > 0) {
